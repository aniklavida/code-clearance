...
- Human-required classes (`human_required_classes` in policy config by rule ID, tool, or severity) guarantee that findings matching these criteria cannot be cleared (e.g. rejected or accepted-risk) by an agent or tool reviewer. An agent attempting to clear such findings will leave the blocking policy unchanged.
