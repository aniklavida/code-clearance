...
1. **Unavailable Required Checks Never Pass:**
   If a required scanner or command is missing from PATH, fails to execute, crashes, times out, or is skipped, the clearance outcome must be `incomplete` in Quick/Full scopes and `blocked` in Release scope. It is structurally impossible to represent an unavailable required check as a pass.
